            ...
            for GBWT in ${GBWTS[@]} ; do
                ...
                for PARAM_PRESET in default fast ; do
                    for PAIRING in single paired ; do 
                        ...
                        vg giraffe -x graph.xg -H graph.gbwt -g graph.gg  -d graph.dist -G sim.gam -b ${PARAM_PRESET} ${PAIRED} -t 24 -p > mapped.gam 
                        ...
                        jq -r '(if .correctly_mapped then 1 else 0 end|tostring) + "," + (.mapping_quality|tostring) + "," + (.score|tostring)' compared.json | sed 's/,/\t/g' | sed "s/$/\tgiraffe_${GRAPH}${GBWT}${READS}${PARAM_PRESET}${PAIRING}/" >> roc_stats.tsv
                    done
                done
            done
        done
