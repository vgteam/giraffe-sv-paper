...
type.df = lapply(paste0('chr', c(1:22, 'X', 'Y')), function(chrn){
  message(chrn)
  svs = readSVvcf(paste0('hsvlr-fordedup-', chrn, '.vcf'), vcf.object=TRUE, keep.ids=TRUE)
  tibble(id=names(svs), type=info(svs)$SVTYPE)
})
type.df = do.call(rbind, type.df)

## cluster info
cl.df = read.table('neardups-clusters.tsv', as.is=TRUE, header=TRUE)
...
write.table(calls.cl, file='hsvlr-srdedup17-support.tsv', sep='\t', quote=FALSE, row.names=FALSE)
...
pdf('hsvlr-srdedup17-support.pdf', 9, 7)
...
write.table(calls.cl.s, file='hsvlr-srdedup17-support-summary.tsv', row.names=FALSE, sep='\t')
