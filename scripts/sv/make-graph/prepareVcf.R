...
## HSVLR catalog
svs = lapply(paste0('chr', c(1:22, 'X', 'Y')), function(chrn){
  message(chrn)
  readSVvcf(paste0('../remap-to-dedup-merged-svs/hsvlr-fordedup-', chrn, '.vcf'), vcf.object=TRUE, keep.ids=TRUE)
})
svs = do.call(rbind, svs)

## Remove near-duplicates that were less supported by short-reads
calls.cl = read.table('../remap-to-dedup-merged-svs/hsvlr-srdedup17-support.tsv', as.is=TRUE, header=TRUE)
...
names(svs) = paste0('sv', 1:length(svs), '_', info(svs)$SIZE)
...
## chr lengths
if(!file.exists('hg38-cytoBandIdeo.txt.gz')){
  download.file('https://hgdownload.soe.ucsc.edu/goldenPath/hg38/database/cytoBandIdeo.txt.gz', 'hg38-cytoBandIdeo.txt.gz')
}
cyto.df = read.table('hg38-cytoBandIdeo.txt.gz', as.is=TRUE, sep='\t')
cyto.df = cyto.df[, 1:3]
colnames(cyto.df) = c('chr', 'start', 'end')

aug.dir = 'chunks'
chunks.df = lapply(paste0('chr', c(1:22, 'X', 'Y')), function(chrn){
...
write.table(chunks.df, file='hsvlr_srdedup17_augment_chunks.bed', row.names=FALSE, col.names=FALSE, sep='\t', quote=FALSE)

## Write VCF to convert to fasta for augmentation
info(svs.for.aug)$GT = info(svs.for.aug)$END = info(svs.for.aug)$QUAL = info(svs.for.aug)$SIZE = NULL
names(geno(svs.for.aug)) = character()
writeVcf(svs.for.aug, filename='hsvlr_srdedup17_foraug.vcf')

## Write VCF for construct
info(svs.for.cons)$GT = info(svs.for.cons)$END = info(svs.for.cons)$QUAL = info(svs.for.cons)$SIZE = NULL
names(geno(svs.for.cons)) = character()
writeVcf(svs.for.cons, filename='hsvlr_srdedup17_forcons.vcf')
